...
def leaderboard_query() -> Response | tuple[str, int]:
    ...
    res = cur.execute(f"""
        ...
        ORDER BY
            ...
            c.id ASC
    """).fetchall()
    boards = cur.execute("SELECT b.id, b.name, (CASE b.owner=? WHEN 1 THEN b.cookie ELSE '' END), (b.last_updated>=?) FROM joinedboards j JOIN boards b ON j.board = b.id WHERE j.clicker = ? ORDER BY j.board ASC", (cid,timestamp,cid,)).fetchall()
